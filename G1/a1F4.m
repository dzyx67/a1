%% a1F4:此程序为调用的函数，为改进的线性权重粒子群 LPSO 的算法
%{
...
%% 函数主体
function [Best_pos,Best_score,Convergence_curve] = a1F4(net)
...
if nargin == 1                                                                 % 确保函数的输入变量的个数为 4 个
    ...
    m = find(lossA == lossC,1);                                                 % 找到适应度最小的那个粒子的下标为 m evalin   assignin
    
    gbest = x(m,:);                                                             % 定义最佳的粒子为 gbest
    
    %% 开始粒子的更新迭代
    Convergence_curve = lossC .* ones(maxgen,1);                                 % 初始化每次迭代得到的最佳的适应度
    h0 = waitbar(0,'1','Name','LPSO-ANN optimization...',...
    ...
    for d = 1:1:maxgen                                                            % 开始迭代，一共迭代 maxgen 次
        ...
        for i = 1:1:popsize                                                       % 依次更新第i个粒子的速度与位置
            ...
            lossA(i,:) = a1F3(x(i,:),net);                                        % 更新第 i 个粒子在第 d 代中的适应度为 lossA_pso1(i,:)
